  ...
  return (
    <section id="contact" className="py-16 md:py-24 bg-nexlead-dark text-white">
      <div className="container-custom">
        <div ref={ref} className={`text-center mb-16 animate-on-scroll ${inView ? 'is-visible' : ''}`}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            {!otpSent ? (
              <>
                <div>
                  ...
                  <input
                    id="name"
                    type="text"
                    className={`input-field text-black ${errors.name ? 'border-red-500 ring-red-500' : ''}`}
                ...
                <div>
                  ...
                  <input
                    id="email"
                    type="email"
                    className={`input-field text-black ${errors.email ? 'border-red-500 ring-red-500' : ''}`}
              ...
              <div>
                ...
                <label htmlFor="otp" className="block mb-2 font-medium">Verification Code*</label>
                <input id="otp" type="text" className="input-field text-black" placeholder="Enter the 6-digit code" value={otp} onChange={(e) => setOtp(e.target.value)} required />
              </div>
            )}
            
            <button type="submit" className="btn-primary w-full" disabled={isSubmitting}>
              {isSubmitting ? 'Processing...' : otpSent ? 'Verify & Submit' : 'Get Started'}
            </button>
          </form>
        </div>
      </div>
